export function mount(container: HTMLElement, api: any): void {
  container.innerHTML = `
    <div style="padding: 24px; color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 1000px; margin: 0 auto;">
      ...
      <div id="ag-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(380px, 1fr)); gap: 16px;">
        <div style="color: #94a3b8; font-size: 14px;">Загрузка данных квот...</div>
      </div>
    </div>
  `;
  ...
  async function loadData(): Promise<void> {
    ...
    const accountEl = container.querySelector('#ag-account') as HTMLElement;
    if (!grid) return;
    ...
    try {
      ...
      const models: { name: string; remaining: string; resetsIn: string }[] = [];
      ...
      if (models.length > 0) {
        grid.innerHTML = models.map((m: any) => {
